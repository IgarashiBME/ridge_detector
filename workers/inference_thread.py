...
Writes results to SharedState instead of Qt signals.
"""
...
class InferenceThread(threading.Thread):
    ...
    def reload_model(self, model_path: str):
        """Schedule model reload on next inference loop iteration."""
    ...
    def _open_serial(self):
        if not serial_available or self.serial_port is None:
            if self.serial_port is not None and not serial_available:
                self._state.append_log("pyserial not installed. Serial disabled.")
            return
        try:
            ...
            self._state.append_log(
                f"Serial opened: {self.serial_port} @ {self.serial_baud}bps"
            )
        except Exception as e:
            self._state.append_log(f"ERROR: Serial open failed: {e}")
            self._serial_conn = None
    ...
    def _close_serial(self):
        if self._serial_conn is not None:
            try:
                ...
                pass
            self._state.append_log(
                f"Serial closed. Sent: {self._serial_send_count}, "
                f"Errors: {self._serial_error_count}"
            )
            self._serial_conn = None
    ...
    def _send_serial(self, msg: bytes):
        ...
        try:
            ...
            if self._serial_error_count % 10 == 1:
                self._state.append_log(f"Serial error: {e}")
    ...
    def _check_model_reload(self):
        ...
        self._state.append_log(f"Reloading model: {path}")
        try:
            from ultralytics import YOLO
            self._model = YOLO(path)
            self.model_path = path
            self._state.append_log(f"Model reloaded: {path}")
        except Exception as e:
            self._state.append_log(f"ERROR: Model reload failed: {e}")
    ...
    def run(self):
        # Load YOLO model
        self._state.append_log(f"Loading YOLO model: {self.model_path}")
        try:
            from ultralytics import YOLO
            self._model = YOLO(self.model_path)
            self._state.append_log("YOLO model loaded.")
        except Exception as e:
            self._state.append_log(f"ERROR: Failed to load YOLO model: {e}")
            return
            ...
            try:
                ...
                if ab is not None:
                    ...
                    if self._filtered_a is None:
                        ...
                        self._filtered_a = (
                            self.ema_alpha * a_raw
                            + (1 - self.ema_alpha) * self._filtered_a
                        )
                        self._filtered_b = (
                            self.ema_alpha * b_raw
                            + (1 - self.ema_alpha) * self._filtered_b
                        )
                ...
                self._state.set_annotated_frame(vis_frame)
                ...
                self._state.set_detection(
                    a=a_val,
                    b=b_val,
                    fps=fps_disp,
                    infer_time_ms=infer_time_ms,
                    serial_status=serial_status_str,
                    serial_count=self._serial_send_count,
                )

            except Exception as e:
                self._state.append_log(f"Inference error: {e}")
                time.sleep(0.1)

        # Cleanup
        self._close_serial()
        self._state.append_log("Inference thread stopped.")
